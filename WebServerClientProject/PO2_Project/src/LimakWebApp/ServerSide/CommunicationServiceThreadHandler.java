...
public class CommunicationServiceThreadHandler extends ServicesHandler {
     ...
     * @param controller {@link AbstractServerController} to perform communication with server's GUI
     * @param ID unique ID for session, generated by {@link MainPageController}
     * @throws IOException if handler is not valid
     */
    public CommunicationServiceThreadHandler(SocketHandler handler, CredentialPacket packet, String ID, AbstractServerController controller) throws IOException{
        super(handler, (controller).getCredentialPacket(), ID);
        ...
        ((AbstractServerController)this.mainPageController).putId(ID);
        Runnable task = ()->{
          initCommunication();
        };
    ...
    /**
     * This method handles received {@link MessageToSend} object to perform action
        ...
        switch(command.getCommandType()){
            ...
            case TRANSFER_FILE:{
                if(command.getContents().size() == 1 && isSomethingToShare == true && itemToShare.isEmpty() == false){
                    ...
                    String dir = ((AbstractServerController)mainPageController).findFileInServer(itemToShare);
                    File item = new File(dir,itemToShare);
                    toSend.add(item);
            ...
            case SEND_LIST_OF_FILES: {
                ...
                newMessage.addContents(new ArrayList<>(((AbstractServerController)mainPageController).getListOfFilesForUser(command.getUser())));
            ...
            case RECEIVE_LIST_OF_FILES:{
                ...
                filesToTransfer = ((AbstractServerController)mainPageController).compareUserAndServerList(command.getUser(), tmp);
            ...
            case GIVE_LIST_OF_ACTIVE_USERS: {
                ...
                newMessage.addContents(new ArrayList<>(((AbstractServerController)mainPageController).getActiveListOfClients()));
                getNotificationService().sendObject(newMessage);
                break;
            }
            case GIVE_LIST_OF_INACTIVE_USERS:{
                ...
                newMessage.addContents(new ArrayList<>(((AbstractServerController)mainPageController).getInactiveListOfClients()));
                getNotificationService().sendObject(newMessage);
                break;
            }
            case SHARE_FILE_TO_USER:{
                String userName = command.getContents().get(0) instanceof String ? (String)command.getContents().get(0) : "";
                CredentialPacket packet = ((AbstractServerController)mainPageController).findUserByName(userName);
                String fileName = command.getContents().get(1) instanceof String ? (String)command.getContents().get(1): "";
                ((AbstractServerController)mainPageController).shareFile(packet, fileName);
                break;
            }
            case REMOVE_USER_FROM_FILE_OWNERS:{
                ...
                String fileName = command.getContents().get(1) instanceof String ? (String)command.getContents().get(1): "";
                ((AbstractServerController)mainPageController).removeUserFromFileOwners(fileName, packet);
                break;
            }
        }
    }
    ...
    /**
     * This method saves provided {@link FilePacket} on {@link Server} workspace
    ...
    protected void saveFile(FilePacket filePacket){
        ...
        boolean ifAlreadyNotExists = ((AbstractServerController)mainPageController).findFileInServer(filePacket.getFileName()).equals("") ;
        try {
            if (!ifAlreadyNotExists) throw new FileAlreadyExistsOnServerException(mainPageController);
            if (root.exists() && root.listFiles() != null) {
                ...
                CredentialPacket packet = (((AbstractServerController) mainPageController)).findUserByName(filePacket.getUserName());
                (((AbstractServerController) mainPageController)).getDisk(idx).putOwnerToFile(filePacket.getFileName(), packet);
                (((AbstractServerController) mainPageController)).addToRuntimeMap(packet, filePacket.getFileName());
        ...
        }catch(FileAlreadyExistsOnServerException io){
            io.log(filePacket.getFileName());
        }
    }

    /**
     * This method sends provided list of files to connected user
     ...
     */
    @Override
    protected void sendListOfFiles(Map.Entry<CredentialPacket, ArrayList<File>> data) {
        if (data == null || data.getValue().size() == 0 || data.getKey() == null) return;
        int filesSent = 0;
        for (File file : data.getValue()) {
            String dir = (((AbstractServerController)mainPageController)).findFileInServer(file.getName());
            if (dir.equals("")) {
            ...
            try {
                FilePacket packet = new FilePacket(data.getKey().getUserName(), file);
